...
给具备图像生成能力的 Agent 加上一层 design-director judgment，让结果更接近真实用户会采用的图像成品，而不只是“能出图”。
...
AI Agent 原本常见的问题不是不会写 prompt，而是缺少一层更稳定的判断：什么时候该直接生成，什么时候该先补关键信息，什么时候该只做最小修正，什么时候其实是前面对需求理解错了。`image2-design-director` 补上的就是这层分阶段判断、质量把关和经验复利机制。
...
- 海报、宣传图、品牌图、公开发布图
- 社交媒体配图、封面图、feed creative
- 产品图、商品图、设备主视觉、mockup
- UI、UX、App screen、dashboard、design system visual
- 插画、场景图、概念图、onboarding visual
...
| 结构化 prompt 组装 | 会把模糊需求整理成更稳定的生成输入，而不是只做表面润色 |
| 设计质量判断 | 会判断这张图到底是不是用户真正要的那种图，而不是只看“漂不漂亮” |
| 交付阶段支持 | 支持从原始生成图继续推进到 text-safe visual、delivery-ready visual 和多尺寸版本 |
| 经验沉淀 | 会记录高价值 case、失败原因和修正方式，让后续类似任务更稳 |

## 它是怎么工作的

这个 skill 的特色，不是“有一段更长的 prompt”，而是它有一套完整的分阶段机制。
...
不是因为它预设了“某一种图”的最佳 prompt，而是因为它用了几层更通用的稳定器：
...
- 它关注的是“这是不是用户真正要的那张图”，不是只关注 prompt 表面工艺
