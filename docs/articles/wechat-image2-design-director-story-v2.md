...
需要二维码、Logo、Exact copy、锁定数值替换和多尺寸适配的，才进入 `visual_base_plus_post` 或局部后期。

高事实敏感、带精确数值和时间口径的，先走 reliability gate，再优先让 Image2 直出完整成品；只有已锁定的局部值需要逐字逐数一致时，才进入 `hybrid_visual_plus_deterministic_overlay`。
